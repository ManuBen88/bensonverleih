...
function sendInvoice_(order, manual, invoiceNo){ const body=buildInvoicePreview_(order, invoiceNo); const owner=Session.getActiveUser().getEmail(); MailApp.sendEmail(owner,'Rechnung '+order.orderId,body); if(order.email) MailApp.sendEmail(order.email,'Ihre Rechnung '+order.orderId,body); }
...
function getServerItems_(){ return [{id:'zelt_4x12',day:80},{id:'zelt_4x6',day:30},{id:'zelt_5x6',day:30},{id:'garnitur_70x220',day:10},{id:'stehtisch_70',day:5}]; }
